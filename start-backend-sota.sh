...
# This script only launches the background servers, allowing both Aider and Odysseus to use them.
...
pkill -9 -f litellm
pkill -9 -f headroom
fuser -k -9 /dev/nvidia0 /dev/nvidia1 8080/tcp 8081/tcp 8082/tcp 4000/tcp 8787/tcp 5000/tcp 2>/dev/null || true
sleep 2
...
if ! (echo > /dev/tcp/localhost/8787) >/dev/null 2>&1; then
    ...
    # Embeddings (3060ti) - Bound to 0.0.0.0 so Docker can reach it!
    ...
    # Headroom Automatic Janitor Proxy - Bound to 0.0.0.0 so Docker can reach it!
    ...
    echo "Initializing AI models sequentially (Wait 30s)..."
    sleep 30
else
